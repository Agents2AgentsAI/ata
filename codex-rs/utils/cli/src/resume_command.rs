//! Shared formatting for user-facing `ata resume` command hints.
...
pub fn resume_hint(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    match thread_name.filter(|name| !name.is_empty()) {
        Some(thread_name) => Some(format!(
            "ata resume, then select {thread_name} ({thread_id})"
        )),
...
mod tests {
    ...
    fn resume_hint_names_picker_item_with_id() {
        ...
        assert_eq!(
            hint,
            Some(
                "ata resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)"
                    .to_string()
            )
        );
    }
