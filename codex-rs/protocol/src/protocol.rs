...
pub enum Op {
    ...
    UserInputWithTurnContext {
        ...
        /// When set, the API key for this provider will be used.
        #[serde(skip_serializing_if = "Option::is_none")]
    ...
    OverrideTurnContext {
        ...
        /// Updated model provider (e.g., "openai", "anthropic", "gemini").
        /// When set, the API key for this provider will be used.
        #[serde(skip_serializing_if = "Option::is_none")]
        model_provider: Option<String>,
...
/// privileges of the agent (e.g. `.ata`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Make .agents/skills and .ata/config.toml and related files read-only
    // to the agent, by default.
    for subdir in &[".agents", ".ata"] {
        #[allow(clippy::expect_used)]
...
mod tests {
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".ata")).expect("create .ata");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".ata"))
