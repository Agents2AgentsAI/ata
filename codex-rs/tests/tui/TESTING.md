...
| TR-038 E, TR-038 E2, TR-044 A, TR-044 B, TR-044 D (`/side`) | `/side` crashes ata with a stack overflow on macOS aarch64 after the v0.134 upstream merge. Submitting `/side` triggers `refresh_in_memory_config_from_disk` (introduced by upstream PR #22106 "Fix side conversation config inheritance"), and the rebuild path recurses infinitely through `ConfigBuilder::build` → `build_inner` → `load_config_with_layer_stack` → (something after `load_agent_roles`) → back to `build`. Each iteration burns ~600KB stack; ~50 iterations overflow even with `RUST_MIN_STACK=32000000`. The bug is in upstream code (not ATA-specific additions). Tests fail with cascading "can't find pane" errors because the runner's tmux session dies. Tracking as a follow-up to the v0.134 merge — to repro: `target/debug/ata --yolo` → type `/side test` → press Enter → stderr shows `thread 'main' has overflowed its stack`. |
