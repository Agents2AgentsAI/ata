...
use agent_jobs::create_report_agent_job_result_tool;
use agent_jobs::create_spawn_agents_on_csv_tool;
use integrations::create_list_mcp_resource_templates_tool;
use integrations::create_list_mcp_resources_tool;
use integrations::create_read_mcp_resource_tool;
...
use workspace::create_test_sync_tool;
use workspace::create_view_image_tool;

const SEARCH_TOOL_BM25_DESCRIPTION_TEMPLATE: &str =
    include_str!("../../templates/search_tool/tool_description.md");
const TOOL_SEARCH_DESCRIPTION_TEMPLATE: &str = SEARCH_TOOL_BM25_DESCRIPTION_TEMPLATE;
...
pub(crate) struct ToolsConfig {
    ...
    /// Feature flags for controlling tool registration.
...
/// A tool provided by an external crate, consisting of a spec (visible to
/// the model) and a handler (invoked when the model calls the tool).
#[allow(private_interfaces)]
pub struct ExternalTool {
    /// Tool specification sent to the model.
    pub spec: ToolSpec,
    /// Handler invoked when the model calls this tool.
    pub(crate) handler: std::sync::Arc<dyn crate::tools::registry::AnyToolHandler>,
}
...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    crate::tools::handlers::attach_url_files::register_attach_url_files(&mut builder, config);
...
pub(crate) fn build_specs_with_toolkits(
    ...
    discoverable_tools: Option<Vec<DiscoverableTool>>,
    dynamic_tools: &[DynamicToolSpec],
    research_toolkit: Option<&std::sync::Arc<SharedResearchToolkit>>,
    data_toolkit: Option<&std::sync::Arc<SharedDataToolkit>>,
    #[cfg(any(feature = "lsp", feature = "treesitter"))] multi_root_state: Option<
        &std::sync::Arc<crate::state::MultiRootState>,
    >,
) -> ToolRegistryBuilder {
    ...
    // Build the base registry with all tools EXCEPT MCP tools, which we handle
    // below so that research/data native tools can suppress their MCP equivalents.
    let mut builder = build_specs_with_discoverable_tools(
        config,
        None, // mcp_tools handled below with suppression
        app_tools,
        discoverable_tools,
        dynamic_tools,
    );
...
/// Like [`build_specs_with_toolkits`] but also registers externally-provided
/// tools at the end of the tool list.
#[allow(clippy::too_many_arguments)]
pub fn build_specs_with_toolkits_and_external(
    config: &ToolsConfig,
    mcp_tools: Option<HashMap<String, rmcp::model::Tool>>,
    app_tools: Option<HashMap<String, ToolInfo>>,
    discoverable_tools: Option<Vec<DiscoverableTool>>,
    ...
    let mut builder = build_specs_with_toolkits(
        config,
        mcp_tools,
        app_tools,
        discoverable_tools,
    ...
    // Register externally-provided tools last so they can override nothing
    // and sit at the end of the tool list.
    for ext in external_tools {
        let name = ext.spec.name().to_owned();
        builder.push_spec(ext.spec.clone());
        builder.register_any_handler(name, std::sync::Arc::clone(&ext.handler));
    }
