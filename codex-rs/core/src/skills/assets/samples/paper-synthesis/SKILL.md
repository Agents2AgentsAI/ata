...
When the synthesis is complete, call `present_document` to present the report in sectioned reading mode. Set `document_id` to a unique slug, `title` to the paper title or synthesis name, and `content` to the full markdown output. End your response after calling this tool and wait for user interaction.
