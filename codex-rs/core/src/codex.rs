...
// Zotero developer instructions removed — the `$zotero` skill provides
// equivalent guidance and is loaded on demand instead of every session.
...
pub(crate) struct SessionConfiguration {
    ...
    features_override: Option<ManagedFeatures>,
...
impl SessionConfiguration {
    ...
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> ConstraintResult<Self> {
        ...
        if let Some(provider_id) = &updates.model_provider
            && let Some(provider) = crate::model_provider_info::built_in_model_providers(None)
                .get(provider_id)
                .cloned()
        {
...
impl Session {
    ...
    async fn build_settings_update_items(
        ...
        // Reading view display mode is injected in build_initial_context and
        // does not need re-injection on every turn. If the display mode changes
        // mid-session, a dedicated update should be emitted at that point.
    ...
    pub(crate) async fn build_initial_context(
        ...
        // Zotero instructions are now provided by the `$zotero` skill on demand.
        ...
        if let Some(coordination_instructions) = self.services.plus.developer_instructions_text() {
        ...
        if turn_context.features.enabled(Feature::ReadingView)
            && self.document_cache.display_mode()
                != crate::tools::handlers::ReadingViewDisplayMode::default()
        {
            developer_sections.push(
                crate::tools::handlers::reading_view_display_mode_state(
                    self.document_cache.display_mode(),
                )
                .to_string(),
            );
...
pub(crate) async fn run_turn(
    ...
    let mut coord_tracker = sess.services.plus.new_tracker();
        ...
        .await
        {
            ...
            Err(CodexErr::ContextWindowExceeded) => {
                ...
                if has_compactable_history {
                    ...
                    if run_auto_compact(&sess, &turn_context, InitialContextInjection::DoNotInject)
                        .await
                        .is_err()
                    {
                        return None;
...
async fn run_sampling_request(
    ...
    // Log ground-truth prompt snapshot for context analysis.
    if let Some(rollout_path) = sess.current_rollout_path().await
        && let Some(session_dir) = rollout_path.parent()
    {
        let session_id = sess.conversation_id.to_string();
        let model = turn_context.model_info.slug.clone();
        let tid = turn_context.sub_id.to_string();
        crate::prompt_snapshot::log_prompt_snapshot(
            &prompt,
            &model,
            &tid,
            session_dir,
...
pub(crate) async fn built_tools(
    ...
    let expose_app_tools_directly = !tools_config.search_tool
        || app_tools
            .as_ref()
    ...
    Ok(Arc::new(ToolRouter::from_config_with_toolkits(
        ...
        #[cfg(any(feature = "lsp", feature = "treesitter"))]
        sess.services.multi_root_state.as_ref(),
    )))
}
...
async fn try_run_sampling_request(
    ...
    let mut response_state = StreamingResponseState::new(plan_mode, &turn_context.sub_id);
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(item) => {
                if let Some(tool_future) = response_state
            ...
            ResponseEvent::OutputItemAdded(item) => {
                response_state
                    .handle_output_item_added(&sess, &turn_context, item)
                    .await;
            }
            ...
            ResponseEvent::Completed {
                ...
                response_state.flush_all(&sess, &turn_context).await;
            ...
            ResponseEvent::OutputTextDelta(delta) => {
                response_state
                    .handle_output_text_delta(&sess, &turn_context, delta)
                    .await;
            }
            ResponseEvent::ReasoningSummaryDelta {
                ...
                response_state
                    .handle_reasoning_summary_delta(&sess, &turn_context, delta, summary_index)
                    .await;
            }
            ResponseEvent::ReasoningSummaryPartAdded { summary_index } => {
                response_state
                    .handle_reasoning_summary_part_added(&sess, &turn_context, summary_index)
                    .await;
            }
            ResponseEvent::ReasoningContentDelta {
                ...
                response_state
                    .handle_reasoning_content_delta(&sess, &turn_context, delta, content_index)
                    .await;
            }
        }
    };
    ...
    if response_state.should_emit_turn_diff() {
        let unified_diff = {
