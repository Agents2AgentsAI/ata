...
fn sanitize_stale_file_references(item: &ResponseItem) -> ResponseItem {
    // ATA-only file attachment sanitization (UrlFile/InputFile) is not available
    // in this build because those ContentItem variants were not merged from upstream.
    item.clone()
}
