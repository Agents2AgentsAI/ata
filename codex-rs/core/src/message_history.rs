...
//!
//! The history is stored at `~/.ata/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.ata`.
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.ata/history.jsonl` and ensure the parent directory exists.
