...
fn absolute_path(path: &Path) -> AbsolutePathBuf {
    // Resolve macOS symlinks like /var -> /private/var by canonicalizing
    // the longest existing ancestor, then appending the remaining suffix.
    ...
    let resolved = dunce::canonicalize(&existing).unwrap_or(existing);
    let full = if suffix.as_os_str().is_empty() {
        resolved
    } else {
        resolved.join(suffix)
    };
    match AbsolutePathBuf::try_from(full.as_path()) {
        Ok(path) => path,
...
"#,
    ...
    );
    {
        ...
        let expected_path = dunce::canonicalize(&expected_path).unwrap_or(expected_path);
        assert_eq!(
            approval.skill_metadata,
            Some(ExecApprovalRequestSkillMetadata {
                path_to_skills_md: expected_path,
            })
        );
    }
...
"#,
    ...
    // wait_for_exec_approval_request already consumed the TurnComplete event
    // when it returned None, so we must not wait for it again.
