...
/// Returns `true` if the `dotslash` CLI is available on `$PATH`.
pub fn is_dotslash_available() -> bool {
...
#[macro_export]
macro_rules! skip_if_no_dotslash {
    () => {{
        if !$crate::is_dotslash_available() {
            eprintln!("Skipping test because `dotslash` is not installed.");
            return;
        }
    }};
    ($return_value:expr $(,)?) => {{
