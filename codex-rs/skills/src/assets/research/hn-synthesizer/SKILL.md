...
3. **Write a staging file** via `exec_command` (the staging directory is always used for temporary data transfer, regardless of whether KB persistence is enabled):
   ```
   mkdir -p ~/.ata/staging && cat <<'CARD_EOF' > ~/.ata/staging/hn-<thread_id>.md
   ---
   thread_id: "<story_id>"
   title: "<thread title>"
   ...
   CARD_EOF
   ```
4. Return **only the staging file path** (e.g., `~/.ata/staging/hn-46990729.md`). Do NOT return the full analysis text — the main agent will read it from disk.
