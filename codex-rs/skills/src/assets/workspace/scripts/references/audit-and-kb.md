...
ata workspace audit-query --workspace "$WID"

# With filters
ata workspace audit-query --workspace "$WID" --ops "repo_add,repo_remove" --limit 50
ata workspace audit-query --workspace "$WID" --since 1700000000 --until 1710000000
```
...
```bash
ata workspace resolve '@kb' --workspace "$WID"
```

### KB Write Operations (with locking)

```bash
ata workspace run-locked --level kb --workspace "$WID" -- <kb-write-command>
```

## Locking

`ata workspace` supports fine-grained locking via `run-locked`. Lock ordering: workspace < kb < run < index.

| Lock | Path | Usage |
|------|------|-------|
| Workspace | `locks/workspace.lock` | Manifest mutations (automatic via typed mutation commands) |
