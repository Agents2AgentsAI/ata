...
pub enum Command {
    /// Submit a new Ata Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of an Ata Cloud task.
    Status(StatusCommand),
    /// List Ata Cloud tasks.
    List(ListCommand),
    /// Apply the diff for an Ata Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for an Ata Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Ata Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `ata cloud` to browse).
    ...
    /// Git branch to run in Ata Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Ata Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Ata Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Ata Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
