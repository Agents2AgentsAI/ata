...
pub struct AuthDotJson {
    ...
    /// Per-provider credentials added by ATA's multi-provider auth layer.
    /// Empty for upstream-style files; populated for any provider that has
    /// gone through `login_with_provider_*`.
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub providers: HashMap<String, ProviderCredential>,
}

/// OAuth credential payload stored under `AuthDotJson::providers`.
...
/// Credential variant stored for a single provider in `AuthDotJson::providers`.
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq)]
...
/// Stable provider id for OpenAI credentials (used here for the legacy
/// `OPENAI_API_KEY` mirroring inside the multi-provider methods).
const PROVIDER_OPENAI_ID: &str = "openai";
...
impl AuthDotJson {
    pub fn migrate_if_needed(mut self) -> Self {
        ...
        if let Some(api_key) = self.openai_api_key.clone()
            && !self.providers.contains_key(PROVIDER_OPENAI_ID)
        {
            self.set_provider_credential(
                PROVIDER_OPENAI_ID,
                ProviderCredential::Api { key: api_key },
            );
        }
    ...
    pub fn set_provider_credential(&mut self, provider_id: &str, credential: ProviderCredential) {
        ...
        if provider_id == PROVIDER_OPENAI_ID {
        ...
        if changed {
            if provider_id == PROVIDER_OPENAI_ID {
                self.openai_api_key = self
                    .get_provider_api_key(PROVIDER_OPENAI_ID)
