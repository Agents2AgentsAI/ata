...
impl LoginServer {
    ...
    /// Crate-private constructor used by sibling login modules
    /// (currently `gemini_server`) that build a `LoginServer` directly from
    /// a `JoinHandle` + a `ShutdownHandle`.
...
impl ShutdownHandle {
    ...
    /// Crate-private constructor — see `LoginServer::from_parts`.
