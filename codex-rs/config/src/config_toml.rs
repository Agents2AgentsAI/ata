...
pub struct ConfigToml {
    ...
    /// ATA: configuration for the ElevenLabs TTS/STT API. Surfaced today as
    /// a config-only entry so users can populate the key/voice ahead of the
...
/// ATA: [elevenlabs] config section. Captures API key, voice, model, etc. so
