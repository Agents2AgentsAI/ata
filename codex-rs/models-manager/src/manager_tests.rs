...
fn build_available_models_picks_default_after_hiding_hidden_models() {
    ...
    // Find the remote models in the merged list and verify their properties.
    let hidden_preset = available.iter().find(|m| m.id == "hidden");
    let visible_preset = available.iter().find(|m| m.id == "visible");
    assert!(
        hidden_preset.is_some(),
        "hidden model should be in available list"
    );
    assert!(
        visible_preset.is_some(),
        "visible model should be in available list"
    );
    assert!(
        !hidden_preset.unwrap().show_in_picker,
        "hidden model should not show in picker"
    );
    assert!(
        visible_preset.unwrap().show_in_picker,
        "visible model should show in picker"
    );

    // Verify a default model is selected from visible models (either the
    // remote "visible" model or one of the built-in presets).
    let has_default = available.iter().any(|m| m.is_default);
    assert!(
        has_default,
        "at least one model should be marked as default"
    );
}
...
fn bundled_third_party_models_json_roundtrips() {
    ...
    let serialized =
        serde_json::to_string(&response).expect("bundled third_party_models.json should serialize");
