...
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "ATA will keep trying on future messages.";
    ...
    pub(crate) fn user_facing_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Send(_) => {
                "ATA could not request IDE context. Try /ide again.".to_string()
            }
            IdeContextError::Read(_) | IdeContextError::InvalidResponse(_) => {
                "ATA could not read IDE context. Try /ide again.".to_string()
            }
        }
    }
    ...
    pub(crate) fn prompt_skip_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "ATA timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("ATA lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("ATA received an unexpected IDE context response.")
            }
            IdeContextError::RequestFailed(_) => hint_with_retry(IDE_DID_NOT_PROVIDE_CONTEXT_HINT),
            IdeContextError::Read(_) => hint_with_retry("ATA could not read IDE context."),
        }
    }
...
mod tests {
    ...
    fn fetch_ide_context_uses_unregistered_request_route() {
        ...
        std::fs::set_permissions(tempdir.path(), std::fs::Permissions::from_mode(0o700))
            .expect("set safe socket directory permissions");
        let socket_path = tempdir.path().join("codex-ipc.sock");
