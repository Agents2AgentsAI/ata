...
impl App {
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        // Build the feature_flags map BEFORE consuming feature_updates_to_apply.
        let feature_flags: std::collections::BTreeMap<String, bool> = feature_updates_to_apply
            .iter()
        ...
        let windows_sandbox_level = if windows_sandbox_changed {
            Some(codex_protocol::config_types::WindowsSandboxLevel::from_config(&self.config))
        } else {
            None
        };
        ...
        {
            // Send an OverrideTurnContext with the feature flags and any
            // guardian approval/sandbox overrides so the live session picks
    ...
    fn spawn_coordination_watcher(&mut self, thread_id: ThreadId) {
        ...
        #[cfg(feature = "ata-plus")]
        ata_plus::on_session_start(&self.config, session_id, move |notification| {
    ...
    pub async fn run(
        ...
        let auth: Option<codex_core::CodexAuth> = auth_manager.auth().await;
        ...
        match event {
            ...
            AppEvent::PersistModelSelection {
                ...
                match ConfigEditsBuilder::new(&self.config.codex_home)
                    .with_profile(profile)
                    .set_model(Some(model.as_str()), effort, provider)
                    .apply()
                    .await
                {
                    Ok(()) => {
            ...
            AppEvent::UpdateVoiceSettings {
                ...
                // Persist to config.
                let startup_edit =
                    codex_core::config::edit::voice_mode_enabled_edit(startup_enabled);
                ...
                let mut edits: Vec<codex_core::config::edit::ConfigEdit> =
                    vec![startup_edit, tts_edit, stt_edit, verbosity_edit];
...
mod tests {
    ...
    async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
        ...
        let submitted_op = op_rx.try_recv().expect("expected OverrideTurnContext op");
        match &submitted_op {
            Op::OverrideTurnContext {
                approval_policy,
                approvals_reviewer,
                ...
                assert_eq!(*approval_policy, Some(guardian_approvals.approval_policy));
                assert_eq!(
                    *approvals_reviewer,
                ...
                // feature_flags must be Some — the feature toggle should propagate
                // to the running session so it takes effect on the next turn.
                assert!(
                    feature_flags.is_some(),
                    "feature_flags must be Some to propagate toggle to session"
                );
                let flags = feature_flags.as_ref().unwrap();
                assert_eq!(
                    flags.get("guardian_approval"),
                    Some(&true),
                    "GuardianApproval flag should be true"
                );
            }
    ...
    -> Result<()> {
        ...
        assert_eq!(app.runtime_approval_policy_override, None);
        let submitted_op = op_rx.try_recv().expect("expected OverrideTurnContext");
        match &submitted_op {
            Op::OverrideTurnContext {
                approval_policy,
                approvals_reviewer,
                ...
                assert!(
                    feature_flags.is_some(),
                    "feature_flags must propagate to session"
                );
                assert_eq!(
                    feature_flags.as_ref().unwrap().get("guardian_approval"),
                    Some(&false),
                    "guardian_approval flag should be false after disabling"
                );
            }
            other => panic!("expected OverrideTurnContext, got {other:?}"),
        }
        ...
        match &submitted_op {
            Op::OverrideTurnContext {
                ...
                assert_eq!(
                    feature_flags.as_ref().unwrap().get("guardian_approval"),
                    Some(&true),
                    "guardian_approval flag should be true after enabling"
                );
            }
            other => panic!("expected OverrideTurnContext, got {other:?}"),
        }
...
"#;
    ...
    -> Result<()> {
        ...
        // The op is sent even when the guardian disable is blocked, but it
        // should carry no policy overrides and an empty feature_flags map
        // (since the feature update was skipped).
        let submitted_op = op_rx.try_recv().expect("expected OverrideTurnContext");
        match &submitted_op {
            Op::OverrideTurnContext {
                approval_policy,
                ...
                assert!(
                    feature_flags.as_ref().unwrap().is_empty(),
                    "feature_flags should be empty when the guardian disable was blocked"
                );
            }
            other => panic!("expected OverrideTurnContext, got {other:?}"),
        }
    ...
    #[tokio::test]
    async fn update_feature_flags_research_toggle_sends_feature_flags_to_session() -> Result<()> {
        let (mut app, _app_event_rx, mut op_rx) = make_test_app_with_channels().await;
        let codex_home = tempdir()?;
        app.config.codex_home = codex_home.path().to_path_buf();

        // Toggle Research off — this should send feature_flags via OverrideTurnContext.
        app.update_feature_flags(vec![(Feature::Research, false)])
            .await;

        assert!(!app.config.features.enabled(Feature::Research));

        let submitted_op = op_rx
            .try_recv()
            .expect("expected OverrideTurnContext op after research toggle");
        match &submitted_op {
            Op::OverrideTurnContext { feature_flags, .. } => {
                assert!(
                    feature_flags.is_some(),
                    "feature_flags must be Some so the session picks up the toggle"
                );
                let flags = feature_flags.as_ref().unwrap();
                assert_eq!(
                    flags.get("research"),
                    Some(&false),
                    "Research flag should be false after toggling off"
                );
            }
            other => panic!("expected OverrideTurnContext, got {other:?}"),
        }
