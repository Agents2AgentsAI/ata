...
pub(crate) struct ChatWidget {
    ...
    /// Cached ElevenLabs API key from the last voice-setup save.
    /// Overrides config for the current session without mutating process env.
    #[cfg(not(target_os = "linux"))]
    ...
    /// Raw section bodies for the browser reading view, preserving `<eq>`
    /// speech text so browser read-aloud can narrate natural language while
    /// the displayed section still renders the LaTeX form.
...
impl ChatWidget {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        if widget
            ...
            .and_then(|voice_config| voice_config.enabled)
            .unwrap_or(false)
        {
    ...
    fn add_team_output(&mut self, args: String) {
        #[cfg(feature = "ata-plus")]
        {
            ...
            tokio::spawn(async move {
                let lines = match ata_plus::render_slash_output(
                    &codex_home,
                    &cwd,
                    &args,
                ...
                {
                    Ok(lines) => lines,
                    Err(e) => vec![Line::from(format!("Error: {e}").red())],
                };
...
// /team command functions have been moved to ata_plus::team_ui
