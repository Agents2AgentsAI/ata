...
// Constants and helpers live in `crate::tts_speed` so the Linux-only
// `tts_linux` module can share the same ladder without depending on this
// voice_mode module (gated off on Linux). Re-exported here so the rest of
// this file refers to them by short name.
#[cfg(test)]
pub(crate) use crate::tts_speed::format_speed;
pub(crate) use crate::tts_speed::snap_to_ladder;
pub(crate) use crate::tts_speed::speaking_status_text;
pub(crate) use crate::tts_speed::step_speed_down;
pub(crate) use crate::tts_speed::step_speed_up;
