...
// Lives in `crate::tts_speed` so the Linux-only `tts_linux` module can share
// the same constants without depending on this voice_mode module (which is
// gated off on Linux). Re-export the helpers used by this file.
...
mod tests {
    use super::*;
    use crate::tts_speed::format_speed;
