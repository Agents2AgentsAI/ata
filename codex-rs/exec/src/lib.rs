...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = enforce_login_restrictions(&config, None) {
        eprintln!("{err}");
        std::process::exit(1);
    }
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    // Suppress unused-variable warnings when the `relay` feature is not enabled.
    let _ = &worker;
    let _ = &lead_session_id;

    // If a workspace is specified, export it as an environment variable so
    // downstream code (e.g. the workspace system) can pick it up.
    if let Some(ref ws) = workspace {
        // SAFETY: we are single-threaded at this point in startup.
        #[allow(unused_unsafe)]
        unsafe {
            std::env::set_var("ATA_WORKSPACE", ws);
        }
    }
    let _ = &workspace;
    ...
    // Worker mode: set up a channel for incoming relay messages that will be
    // submitted as new turns after the initial turn completes.
    #[cfg(feature = "ata-plus")]
    let (worker_msg_tx, mut worker_msg_rx) = tokio::sync::mpsc::channel::<String>(32);
    #[cfg(feature = "ata-plus")]
    let mut shutdown_requested = false;
    #[cfg(feature = "ata-plus")]
    let mut waiting_for_work = false;

    // Create the relay client early so it can be shared between the SSE
    // watcher and the main loop (for posting results back).
    #[cfg(feature = "ata-plus")]
    let worker_relay: Option<std::sync::Arc<codex_coordination::relay_client::RelayClient>> =
        if worker {
            let coordination = config.coordination.as_ref();
            let relay_url = coordination
                .and_then(|c| c.relay_url.clone())
                .unwrap_or_else(|| "http://127.0.0.1:7800".to_string());
            let relay_secret = coordination.and_then(|c| c.relay_secret.clone());

            match codex_coordination::compute_project_id(&config.cwd).await {
                Some(project_id) => Some(std::sync::Arc::new(
                ...
                None => {
                    warn!("[worker] No project ID; relay disabled");
                    None
                }
            }
        } else {
                    ...
                    CodexStatus::InitiateShutdown => {
                        ...
                        if worker && !shutdown_requested {
                            if !waiting_for_work {
                                ...
                                eprintln!(
                                    "[worker] Turn complete. Waiting for directed messages..."
                                );

                                ata_plus::on_turn_complete(
                                    &worker_relay,
                            ...
                            loop {
                                tokio::select! {
                                    msg = worker_msg_rx.recv() => {
                                        match msg {
                                            Some(task_message) => {
                                                eprintln!("[worker] Received task: {}", ata_plus::worker::preview_message(&task_message));
                                                waiting_for_work = false;

                                                let items: Vec<codex_app_server_protocol::UserInput> = vec![
                                                    codex_protocol::user_input::UserInput::Text {
                                                        text: task_message,
                                                        text_elements: Vec::new(),
                                                    }.into()
                                                ];
                                                if let Err(err) = send_request_with_response::<TurnStartResponse>(
                                                    &client,
                                                    ClientRequest::TurnStart {
                                                        request_id: request_ids.next(),
                                                        params: TurnStartParams {
                                                            ...
                                                            approvals_reviewer: None,
                                                            sandbox_policy: Some(config.permissions.sandbox_policy.get().clone().into()),
                                                            model: None,
                                                            service_tier: None,
                                                            effort: default_effort,
                                                            summary: None,
                                                    ...
                                                    eprintln!("[worker] Failed to submit task: {err}");
                                                }
                                                break;
                                            }
                                            None => {
                                                eprintln!("[worker] Relay channel closed. Exiting.");
                                                break;
                                            }
                                        }
                                    }
                                    server_event = client.next_event() => {
                                        // Drain trailing events from the completed turn
                                        // while waiting for the next directed message.
                                        match server_event {
                                            Some(InProcessServerEvent::LegacyNotification(notif)) => {
                                                if let Ok(decoded) = decode_legacy_notification(notif)
                                                    && matches!(decoded.event.msg, EventMsg::Error(_))
                                                {
                                                    error_seen = true;
                                                }
                                            }
                                            None => break,
                                            _ => {} // Continue draining other events.
                                        }
                                    }
                                    _ = tokio::signal::ctrl_c() => {
                                        eprintln!("\n[worker] Received Ctrl-C. Shutting down.");
