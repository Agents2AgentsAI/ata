...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Ata, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejoins_running_thread_even_with_override_mismatch() -> Result<()> {
    ...
    // The thread status may be Active or Idle depending on whether the
    // in-flight turn completed before the resume response was assembled.
    // The important thing is that the resume succeeded for the running thread.
    assert!(
        matches!(
            thread.status,
            ThreadStatus::Active { .. } | ThreadStatus::Idle
        ),
        "expected Active or Idle, got {:?}",
        thread.status,
    );
