...
async fn get_user_agent_returns_current_codex_user_agent() -> Result<()> {
    ...
    let client_version = codex_core::models_manager::OPENAI_MODELS_CLIENT_VERSION;
    let user_agent = format!(
        "{originator}/{client_version} ({os_type} {os_version}; {architecture}) {terminal_ua} ({DEFAULT_CLIENT_NAME}; 0.1.0)"
    );
