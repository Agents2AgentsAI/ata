...
impl CodexMessageProcessor {
        ...
        match params {
            ...
            LoginAccountParams::Gemini => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
                    message: "Gemini login is not supported in this build".to_string(),
                    data: None,
                };
            ...
            .await
        {
            ...
            Err(err) => {
                let (code, message) = match err {
                    ...
                    SteerInputError::InvalidFileInput(msg) => (
                        INVALID_REQUEST_ERROR_CODE,
                        format!("invalid file input: {msg}"),
                    ),
                };
                let error = JSONRPCErrorError {
                    code,
    ...
    async fn get_user_saved_config(&self, request_id: ConnectionRequestId) {
        ...
        let user_saved_config = match service.load_user_saved_config().await {
            Ok(config) => config,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
        ...
        let response = codex_app_server_protocol::GetUserSavedConfigResponse {
    ...
    async fn get_user_info(&self, request_id: ConnectionRequestId) {
        ...
        let response = codex_app_server_protocol::UserInfoResponse { alleged_user_email };
        self.outgoing.send_response(request_id, response).await;
    }
    ...
    async fn set_default_model(
        &self,
        request_id: ConnectionRequestId,
        params: codex_app_server_protocol::SetDefaultModelParams,
    ) {
        let codex_app_server_protocol::SetDefaultModelParams {
            model,
            reasoning_effort,
        } = params;
        match codex_core::config::edit::ConfigEditsBuilder::new(&self.config.codex_home)
        ...
        {
            Ok(()) => {
                let response = codex_app_server_protocol::SetDefaultModelResponse {};
                self.outgoing.send_response(request_id, response).await;
            }
            Err(err) => {
                let error = JSONRPCErrorError {
...
fn thread_id_from_rollout_path(path: &Path) -> Option<ThreadId> {
    ...
    // Only consider rollout files, not prompt-snapshot or other .jsonl files.
